...
from .forms import EditWorksForm, AddExtraImagesForm, EditShopWorksForm, SetShopMessageForm
...
class TestSetShopMessageForm(TestCase):
    def test_set_shop_message_all_fields(self):
        form = SetShopMessageForm({
            'info': 'test',
            'show': True
        })
        self.assertTrue(form.is_valid())
    ...
    def test_fields_are_explicit_in_form_metaclass(self):
        ...
        """
        form = SetShopMessageForm()
        self.assertEqual(form.Meta.fields, ['info', 'show'])
